...
This in-between space is rather interesting, often neglected, and rarely explored. Let's go on a journey and build an image previewer, Somehash, from scratch.
...
This project explored the minimum path to complete the process. Production readiness was not considered. There are some optimizations that can be made for production. One is hiding the preview animation code after the image has loaded and another is choosing a better encoding method. 
