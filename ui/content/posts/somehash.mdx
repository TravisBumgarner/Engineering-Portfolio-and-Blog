...
[Blurhash](https://blurha.sh/) is one such solution. It offers quick loading preview images that do a great job of capturing the attention of visitors until the actual content is ready. 

I love this in-between space. It's a place often neglected and de-prioritized. A place often not explored. Today, I want to pull back the curtain and build my own image previewer, Somehash, from scratch.

## Overview

The journey of an image occurs in three stages: processing, previewing, and loading. First, information is extracted from the image, hashed, and stored. Next, a React component retrieves the hash and renders a preview image. Finally, the full-resolution image is loaded.
...
[KMeans clustering](https://medium.com/analytics-vidhya/color-separation-in-an-image-using-kmeans-clustering-using-python-f994fa398454) is a fascinating algorithm. One application is finding dominant colors from an image. The screenshot below shows an example. 
...
The next step is to transfer information from the Python script to React. The data will be formatted in a way that ensures it can be easily transferred between the two. The encoding process has not been given much consideration. There are definitely better approaches.
...
Additionally, a version field will be added so that different renders can be used.
...
**Preview to Full Image Transition**
...
This project explored the minimum path to complete the process. Production readiness was not considered. There are some optimizations that can be made such as

- Hiding the preview animation after the image has loaded
- Pick the smallest hash size possible. 
